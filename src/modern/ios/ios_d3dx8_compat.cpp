                          ...
                          D3DFORMAT sourceFormat = D3DFMT_UNKNOWN)
{
    ...
    UINT sourceBytes = sourceFormat == D3DFMT_UNKNOWN ? 4 : BytesPerPixel(sourceFormat);
    for (UINT y = 0; y < destinationHeight; ++y)
    {
        ...
        for (UINT x = 0; x < destinationWidth; ++x)
        {
            ...
            const BYTE *pixel = source + sourceY * sourcePitch + sourceX * sourceBytes;
            BYTE rgba[4];
            if (sourceFormat == D3DFMT_UNKNOWN) memcpy(rgba, pixel, 4);
            else DecodePixel(pixel, sourceFormat, rgba);
...
D3DXVECTOR4 *D3DXVec4Transform(D3DXVECTOR4 *out, const D3DXVECTOR4 *value, const D3DXMATRIX *matrix)
{
    if (out == NULL || value == NULL || matrix == NULL) return NULL;
    D3DXVECTOR4 result;
    result.x = value->x * matrix->_11 + value->y * matrix->_21 + value->z * matrix->_31 + value->w * matrix->_41;
    result.y = value->x * matrix->_12 + value->y * matrix->_22 + value->z * matrix->_32 + value->w * matrix->_42;
    result.z = value->x * matrix->_13 + value->y * matrix->_23 + value->z * matrix->_33 + value->w * matrix->_43;
    result.w = value->x * matrix->_14 + value->y * matrix->_24 + value->z * matrix->_34 + value->w * matrix->_44;
    *out = result; return out;
}
...
HRESULT D3DXLoadSurfaceFromMemory(IDirect3DSurface8 *destination, const void *, const RECT *destinationRect,
                                  LPCVOID sourceMemory, D3DFORMAT sourceFormat, UINT sourcePitch,
                                  const void *, const RECT *sourceRect, DWORD, D3DCOLOR colorKey)
{
    if (destination == NULL || sourceMemory == NULL || sourcePitch == 0) return E_INVALIDARG;
    D3DSURFACE_DESC desc; if (FAILED(destination->GetDesc(&desc))) return E_FAIL;
    RECT full = {0, 0, static_cast<LONG>(desc.Width), static_cast<LONG>(desc.Height)};
    const RECT *dst = destinationRect != NULL ? destinationRect : &full;
    UINT width = sourceRect != NULL ? static_cast<UINT>(sourceRect->right) : desc.Width;
    UINT height = sourceRect != NULL ? static_cast<UINT>(sourceRect->bottom) : desc.Height;
    if (sourceFormat == D3DFMT_A8R8G8B8 || sourceFormat == D3DFMT_X8R8G8B8 ||
        sourceFormat == D3DFMT_R8G8B8 || sourceFormat == D3DFMT_R5G6B5 ||
        sourceFormat == D3DFMT_X1R5G5B5 || sourceFormat == D3DFMT_A1R5G5B5 ||
        sourceFormat == D3DFMT_A4R4G4B4)
        // Decode packed D3D pixels, including the 16-bit ANM menu textures.
        // SDL image loading above supplies RGBA and deliberately uses the default.
        return CopyRgbaToSurface(destination, dst, static_cast<const BYTE *>(sourceMemory), width, height, sourcePitch, sourceRect, colorKey, sourceFormat);
    return E_NOTIMPL;
}
