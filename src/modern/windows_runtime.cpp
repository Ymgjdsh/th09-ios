...
bool ShowDataDirectoryError(const wchar_t *message, const wchar_t *path)
{
    ...
    MessageBoxW(NULL, text, L"TH095 reconstruction", MB_OK | MB_ICONERROR);
    return false;
}
...
bool ConfigureDataDirectory()
{
    ...
    attributes = GetFileAttributesW(L"TH095.dat");
    if (attributes == INVALID_FILE_ATTRIBUTES || (attributes & FILE_ATTRIBUTE_DIRECTORY) != 0)
        return ShowDataDirectoryError(L"The selected directory does not contain TH095.dat.", NULL);
    return true;
}
