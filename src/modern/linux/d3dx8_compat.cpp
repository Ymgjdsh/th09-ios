                                  ...
                                  const void *, const RECT *sourceRect, DWORD, D3DCOLOR colorKey)
{
    ...
    UINT width = sourceRect != NULL ? static_cast<UINT>(sourceRect->right - sourceRect->left) : desc.Width;
    UINT height = sourceRect != NULL ? static_cast<UINT>(sourceRect->bottom - sourceRect->top) : desc.Height;
    if (sourceFormat == D3DFMT_A8R8G8B8 || sourceFormat == D3DFMT_X8R8G8B8)
        return CopyRgbaToSurface(destination, dst, static_cast<const BYTE *>(sourceMemory), width, height, sourcePitch, sourceRect, colorKey);
    return E_NOTIMPL;
}
