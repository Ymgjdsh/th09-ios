...
// Canonical TH095 COFF replay needs DIFFBUILD storage/alias semantics while
// retaining the target's C++ decorated namespace symbols.
#define DIFFABLE_EXTERN(type, name) extern type name;
#define DIFFABLE_EXTERN_ARRAY(type, size, name) extern type name[size];
#define DIFFABLE_STATIC(type, name) extern type name;
#define DIFFABLE_STATIC_ARRAY(type, size, name) extern type name[size];
#else
