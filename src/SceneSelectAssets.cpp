...
struct SceneSelectionAssetView
{
    ...
    u8 unknown0000[kFrontEndFlagsOffset];
#else
...
static i32 SceneSelectionPointerToSlot(void *pointer)
{
#if defined(TH095_MODERN_PORT)
    return static_cast<i32>(TH095IosStorePointer(reinterpret_cast<uintptr_t>(pointer)));
#else
    return reinterpret_cast<i32>(pointer);
#endif
}
                                                ...
                                                u8 **value)
{
    if (queue->count < queue->capacity)
    {
        queue->values[queue->count] = SceneSelectionPointerToSlot(*value);
        queue->count++;
    }
}
    ...
    while (true)
    {
        ...
        if (locals.view->stateHistory.count > 0)
        {
            ...
            locals.view->pendingPrimaryData[locals.view->pendingTextureCount] =
                SceneSelectionPointerToSlot(FileSystem::OpenFile(
                    locals.facePath,
                    &locals.view
